...
def syne_trainer(
    ...
    ontix = acx.Ontix(data=acx_container, ontologies=ont_files, config=scconfig)
    ...
    sklearn_ml_regression = (
        linear_model.LinearRegression()
    )  ## Unused, only classification tasks
...
tasks = [
    ...
    "sex",
    "disease",
]  ## Include disease later when available
...
scheduler = CQR(config_space, metric=metric, do_minimize=do_minimize)
...
with open(
    f"./notebooks/large_ontix_save/large_ontix_chatgpt_experiment_{current_date}.pkl",
    "wb",
) as f:
