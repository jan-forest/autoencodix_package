...
class ImageVAEArchitecture(BaseAutoencoder):
    """Optimized VAE with CNN for images, compatible with Fabric and MPS."""
    ...
    def __init__(
        ...
        self.latent_dim = self._config.latent_dim
        self.nc, self.h, self.w = input_dim
        self.img_shape = input_dim
        self.hidden_dim = self._config.hidden_dim
        
        # MPS-specific optimizations
        self._mps_available = torch.backends.mps.is_available()
        self._channels_last = True  # Better memory access pattern
        
        self._build_network()
        self.apply(self._init_weights)
        
        # Convert to channels last if supported and not MPS
        if self._channels_last and not self._mps_available:
            self = self.to(memory_format=torch.channels_last)

    def _build_network(self):
        # Use more efficient conv blocks
        def conv_block(in_c, out_c, transpose=False):
            layers = []
            conv_layer = (
            ...
            if not transpose:  # No batch norm in decoder
                layers.extend([
        ...
        # Build encoder
        encoder_layers = [
            nn.Conv2d(self.nc, self.hidden_dim, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            *[conv_block(self.hidden_dim * (2**i), self.hidden_dim * (2**(i+1))) 
              for i in range(4)]
        ]
        self.encoder = nn.Sequential(*encoder_layers)
        
        # Calculate spatial dimension after encoder
        self.spatial_dim = self.h // (2**5)  # 5 conv layers with stride 2
        ...
        for i in range(4):
            decoder_layers.append(
                conv_block(
                    self.hidden_dim * (2**(4-i)),
                    self.hidden_dim * (2**(3-i)) if i < 3 else self.nc,
                    transpose=True
                )
            )
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        logvar = torch.clamp(logvar, min=0.1, max=20)
    ...
    def forward(self, x: torch.Tensor) -> ModelOutput:
        ...
        z = self.reparameterize(mu, logvar)
        return ModelOutput(
            reconstruction=self.decode(z).view(-1, *self.img_shape),
            latentspace=z,
            latent_mean=mu,
            latent_logvar=logvar,
            additional_info=None,
