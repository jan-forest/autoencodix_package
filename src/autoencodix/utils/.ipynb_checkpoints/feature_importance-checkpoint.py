...
def make_DeepLiftShap_Vanillix_dim(model,inputs,baselines,latent_dimension):
    ...
    attributions,convergence = deeplift.attribute(inputs =inputs, 
                                                  baselines=baselines,
                                                  return_convergence_delta=True)
    avg_abs_attributions = attributions.abs().mean(dim=0)
    return avg_abs_attributions, convergence

def make_feature_importance_Vanillix(van,
                            method = 'DeepLiftShap',
                               n_subset = 100,
                               seed_int =12,
                               baseline_type = 'mean',
    ...
    if baseline_group != 'all' and obs_col is None:
        raise ValueError("obs_col must be provided when using a group-specific baseline.")
    np.random.seed(seed_int)
    torch.manual_seed(seed_int)
    model = van.result.model
    
    input_adata = van.raw_user_data['multi_sc']['multi_sc'].mod['user-data']
    inputs = torch.tensor(input_adata.X.toarray() if scipy.sparse.issparse(input_adata.X) else input_adata.X)
    if baseline_group == 'all':
        ...
        inputs_filtered = torch.tensor(input_adata_filtered.X.toarray() if scipy.sparse.issparse(input_adata_filtered.X) else input_adata_filtered.X)
        if baseline_type == 'mean':
            baseline_mean = inputs_filtered.mean(axis=0) # gene_means 
            baselines = torch.tensor(np.tile(baseline_mean, (inputs.shape[0], 1)))
        if baseline_type == 'random_sample':
            baseline_random = inputs_filtered[torch.randint(0, inputs_filtered.size(0), (1,)).item()]
            baselines = torch.tensor(np.tile(baseline_random, (inputs.shape[0], 1)))
    ...
    indices_DeepLiftShap = np.random.choice(inputs.shape[0], size=n_subset, replace=False)
    all_attr = []
    for latent_dim in latent_dimensions:
        if method == 'DeepLiftShap':
            avg_abs_attributions, convergence =make_DeepLiftShap_Vanillix_dim(model=model,
                                   inputs = inputs[indices_DeepLiftShap].float(),
                                   baselines = baselines[indices_DeepLiftShap].float(),
                                   latent_dimension = latent_dim)
        if method == 'IntegratedGradients':
            avg_abs_attributions, convergence = make_IntegratedGradients_Vanillix_dim(model=model,
                                   inputs = inputs[indices_DeepLiftShap].float(),
                                   baselines = baselines[indices_DeepLiftShap].float(),
                                   latent_dimension = latent_dim)
...
def get_top_kgenes_per_latent_dimension(df_attributions, latent_dim=0, topk=10):
    ...
    """
    topk_genes = df_attributions['latent_dimension_' + str(latent_dim)].nlargest(topk).index.tolist()
    return topk_genes
...
def run_go_enrichment(top_genes_dict,n_top_pathways = 10, gene_set_library="GO_Biological_Process_2021", organism='Human'):
    """
...
def plot_GO_log_odds_all(PE_dict, top_n=10,base_save_identifier = None):
    ...
    for dim, df in PE_dict.items():
        ...
        if base_save_identifier is not None:
            plt.savefig(base_save_identifier + '_' + str(dim) + '.pdf', bbox_inches = 'tight') 
        plt.show()
...
def do_feature_importance_Vanillix(van,
                                   method = 'DeepLiftShap',
                                   baseline_type = 'mean',
                                   baseline_group = 'all',
                                   obs_col = None,
    ...
    df_attributions = make_feature_importance_Vanillix(van,
                                method = method,
                               n_subset = n_subset,
                               seed_int =seed_int,
    ...
    if do_visualizations:
        if save_out_path is not None:
            ...
            plot_union_top_genes_heatmap(df = df_attributions, 
                                     top_n=top_n_genes_heatmap, cmap="plasma",
                                     save = save_path)
        else:
            plot_union_top_genes_heatmap(df = df_attributions, 
                                     top_n=top_n_genes_heatmap, cmap="plasma",
        ...
        dict_top_genes = get_top_genes_per_dimension(df=df_attributions, 
                                                     top_n=top_n_foreground_pathways)
        PE_dict = run_go_enrichment(top_genes_dict = dict_top_genes,
        ...
        if save_out_path is not None: 
            ...
            plot_GO_log_odds_all(PE_dict = PE_dict, 
                         top_n=n_top_pathways,
                         base_save_identifier = base_save_identifier)
        else:
            plot_GO_log_odds_all(PE_dict = PE_dict, 
                         top_n=n_top_pathways,
                         base_save_identifier = None)
    return df_attributions
...
def make_feature_importance_Varix(van,
                               method = 'DeepLiftShap',
                               n_subset = 100,
                               seed_int =12,
                               baseline_type = 'mean',
    ...
    for latent_dim in latent_dimensions:
        if method == 'DeepLiftShap':
            avg_abs_attributions, convergence = make_DeepLiftShap_Varix_dim(model=model,
                                   inputs = inputs[indices_DeepLiftShap].float(),
                                   baselines = baselines[indices_DeepLiftShap].float(),
                                   latent_dimension = latent_dim)
        if method == 'IntegratedGradients':
            avg_abs_attributions, convergence = make_IntegratedGradients_Varix_dim(model=model,
                                   inputs = inputs[indices_DeepLiftShap].float(),
                                   baselines = baselines[indices_DeepLiftShap].float(),
                                   latent_dimension = latent_dim)
...
def do_feature_importance_Varix(van,
                                method = 'DeepLiftShap',
                                baseline_type = 'mean',
                                baseline_group = 'all',
                                obs_col = None,
    ...
    if do_miraculix_vis:
        do_miraculix_visualization()
    df_attributions = make_feature_importance_Varix(van,
                               method = method, 
                               n_subset = n_subset,
                               seed_int =seed_int,
                               baseline_type = baseline_type,
...
def make_IntegratedGradients_Varix_dim(model,inputs,baselines,latent_dimension):
    ...
    attributions,convergence = integrated_gradients.attribute(inputs =inputs, 
                                                  baselines=baselines,
                                                  return_convergence_delta=True)
    avg_abs_attributions = attributions.abs().mean(dim=0)
    return avg_abs_attributions, convergence
