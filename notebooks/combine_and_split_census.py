...
adata_list = [
    load_rename_adata(fp) for fp in file_paths[:]
...
holdout_names, holdout_idx = scanpy.pp.sample(
    np.array(adata.obs_names), fraction=0.05, copy=True, rng=42
)
tune_names, tune_idx = scanpy.pp.sample(
    np.array(adata.obs_names.difference(holdout_names)),
    fraction=0.05,
    copy=True,
    rng=42,
)
...
scanpy.write(
    "./notebooks/large_sc_data/census_holdout_split.h5ad", adata[holdout_names]
)
