...
def default_config():
    return DefaultConfig(
        loss_reduction="mean", reconstruction_loss="mse", default_vae_loss="kl"
    )


@pytest.fixture
def vanillix_loss_module(default_config):
    return VanillixLoss(config=default_config)
...
class TestVanillixLoss:
    def test_mse_loss_initialization(self, default_config, vanillix_loss_module):
        """
        Test that MSE loss is correctly initialized when specified in config.
        """
        loss = vanillix_loss_module
    ...
    def test_bce_loss_initialization(self):
        ...
        loss = VanillixLoss(config=config)
    ...
    def test_mmd_kernel_shape(self, vanillix_loss_module, sample_data):
        """
        Test that MMD kernel output has correct shape (batch_size x batch_size).
        """
        x = sample_data["z"]
        y = torch.randn_like(x)
        kernel = vanillix_loss_module._mmd_kernel(x, y)
        assert kernel.shape == (x.size(0), y.size(0))

    def test_mmd_kernel_bounds(self, vanillix_loss_module, sample_data):
        """
    ...
    def test_mmd_loss_type(self, vanillix_loss_module, sample_data):
        """
        Test that MMD loss returns a scalar tensor.
        """
        z = sample_data["z"]
        true_samples = torch.randn_like(z)
        mmd_loss = vanillix_loss_module.compute_mmd_loss(z, true_samples)
        assert isinstance(mmd_loss, torch.Tensor)

    def test_mmd_loss_shape(self, vanillix_loss_module, sample_data):
        """
    ...
    def test_kl_loss_type(self, vanillix_loss_module, sample_data):
        """
        Test that KL loss returns a scalar tensor.
        """
        kl_loss = vanillix_loss_module.compute_kl_loss(
            sample_data["mu"], sample_data["logvar"]
        )
        assert isinstance(kl_loss, torch.Tensor)

    def test_kl_loss_shape(self, vanillix_loss_module, sample_data):
        """
    ...
    def test_variational_loss_kl_type(self, vanillix_loss_module, sample_data):
        """
        Test that variational KL loss returns a scalar tensor.
        """
        loss = vanillix_loss_module.compute_variational_loss(
            sample_data["mu"], sample_data["logvar"]
        )
    ...
    def test_variational_loss_mmd_type(self, sample_data):
        ...
        loss_module = VanillixLoss(config)
    ...
    def test_forward_total_loss_type(
        self, vanillix_loss_module, sample_data, model_output
    ):
        """
        Test that forward pass returns total loss as a scalar tensor.
        """
        total_loss, _ = vanillix_loss_module(model_output, sample_data["input"])
    ...
    def test_forward_loss_dict_structure(
        ...
        """
        _, loss_dict = vanillix_loss_module(model_output, sample_data["input"])
    ...
    def test_missing_mu_error(self, vanillix_loss_module):
        """
        Test that computing variational loss with missing mu raises ValueError.
        """
        with pytest.raises(ValueError):
            vanillix_loss_module.compute_variational_loss(None, torch.randn(10, 5))

    def test_missing_logvar_error(self, vanillix_loss_module):
        """
        Test that computing variational loss with missing logvar raises ValueError.
        """
        with pytest.raises(ValueError):
            vanillix_loss_module.compute_variational_loss(torch.randn(10, 5), None)
