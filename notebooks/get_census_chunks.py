...
var_value_filter = (
    "feature_id in [" + ", ".join(f'"{gene}"' for gene in gene_names) + "]"
)
...
all_chunks = human_meta_counts.index[
    0:11
]  # Change [0:5] to [:] for all chunks in production
...
for chunk_name in selected_chunks:
    ...
    print(
        f"Number of cells in chunk: {human_meta_counts.loc[chunk_name[0],chunk_name[1],chunk_name[2],chunk_name[3]]}"
    )
    ...
    scanpy.pp.filter_cells(
        adata_chunk, min_genes=int(perc_genes * len(gene_names)), inplace=True
    )
    ...
    scanpy.write(
        folder
        + f"{chunk_name[0].replace(' ', '-')}_{chunk_name[1].replace(':','-').replace(' ', '-')}_{chunk_name[2].replace(' ', '-')}_{chunk_name[3]}_chunk.h5ad",
        adata_chunk,
    )
